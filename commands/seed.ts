...
import { createConnection, factory, getConnectionOptions, importSeed, runSeeder } from 'typeorm-seeding';
...
const run = async () => {
  ...
  let factoryFiles: any;
  let seedFiles: any;
  try {
    factoryFiles = await factory(factoryPath);
    seedFiles = await importSeed(seedsPath);
  } catch (error) {
